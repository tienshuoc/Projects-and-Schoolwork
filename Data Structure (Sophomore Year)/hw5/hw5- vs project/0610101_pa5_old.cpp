...
Route shortest_path(std::vector< std::vector<node>> &graph, int u_row, int u_col, int v_row, int v_col) {
...
int main(int argc, char** argv)
{
	...
	for (std::vector<Vertex>::iterator i = list_of_odd_degree.begin(); i != list_of_odd_degree.end(); i += 2) {
		std::vector<Vertex> temp_track;
		cost += shortest_path(graph, i->x, i->y, (i + 1)->x, (i + 1)->y).cost;
		temp_track = shortest_path(graph, i->x, i->y, (i + 1)->x, (i + 1)->y).track;
