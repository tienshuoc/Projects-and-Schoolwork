...
if not USE_CUDA: print("CUDA IS NOT AVAILABLE!!!")
...
def indexesFromSentence(voc, sentence):
    ...
    return [voc.word2index[word] for word in sentence] + [EOS_token] # no spaces separating words in Chinese corpus
...
def batch2TrainData(voc, pair_batch, reverse):
    ...
    pair_batch.sort(key=lambda x: len(x[0]), reverse=True) # no spaces separating words in Chinese corpus
